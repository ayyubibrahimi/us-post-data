...
def read_personnel_and_employment_hist():
    df = pd.read_csv("../data/input/officer_employment-20231220144255.csv")
...
def split_names(df):
    ...
    pattern = rf"(\w+) (\w+)(?: (\w+))?(?: ({suffixes}))?"

    names = df.full_name.str.strip().str.extract(f"(?i){pattern}")

    df.loc[:, "first_name"] = names[0].fillna("").str.title()
    df.loc[:, "middle_name"] = names[2].fillna("").str.title()
    df.loc[:, "last_name"] = names[1].fillna("").str.title()
    df.loc[:, "suffix"] = names[3].fillna("").str.title()

    df.loc[:, "suffix"] = df.suffix.str.replace(r"Ii", "II", regex=False).str.replace(
        r"Iii", "III", regex=False
    )

    df.loc[:, "full_name"] = df.apply(
        lambda x: " ".join(
            filter(None, [x.first_name, x.middle_name, x.last_name, x.suffix])
        ),
        axis=1,
    ).str.title()
    return df
...
def clean_per_nbr(df):
    df.loc[:, "person_nbr"] = (
        df.person_nbr.str.lower().str.strip().str.replace(r"\s+", "", regex=True)
    )
    return df
...
def read_demo_data():
    df = pd.read_csv("../data/input/officer_data-20231220144255.csv", encoding="latin1")
...
def read_agency():
    df = pd.read_csv("../data/input/agency_data-20231220144255.csv", encoding="latin1")
...
def clean_gt_agcy(df):
    df.loc[:, "agcy_name"] = (
        ...
        .str.replace(r"metro\.", "metro", regex=True)
        .str.replace(
            r"standards-investigation", "standards and investigation", regex=False
        )
        .str.replace(r"(\w+)  (\w+)", r"\1 \2", regex=True)
        .str.replace(r"(.+)? fire (.+)?", "", regex=True)
...
def get_unique_fuzzy_matches(df, threshold):
    ...
    unique_scores = df[(df["fuzzy_score"] < 100) & (df["fuzzy_score"] >= threshold)][
        "fuzzy_score"
    ].unique()
    ...
    for score in unique_scores:
        ...
        results.append(
            {
                "fuzzy_score": score,
                "agcy_name_x": agcy_name_x,
                "agcy_name_y": agcy_name_y,
            }
        )
...
def case_types(df):
    df.loc[:, "type"] = (
        df.type.str.replace(r"^ (\w+)", r"\1", regex=True)
...
if __name__ == "__main__":
    ...
    dfa.to_csv("../data/processed/ga-2024-original-leo.csv", index=False)
    ...
    dfb = read_demo_data()
    dfb.to_csv("../data/processed/ga-2024-original-leo-demographics.csv", index=False)
    ...
    agencies.loc[:, "agcy_uid"] = agencies.agcy_uid.str.replace(
        r"^ (\w+)", r"\1", regex=True
    ).str.replace(r"(\w+) $", r"\1", regex=True)
    agencies = agencies.pipe(clean_gt_agcy)

    merged_df = pd.merge(df, agencies, on="agcy_uid")
    merged_df["fuzzy_score"] = merged_df.apply(
        lambda x: fuzz.ratio(x["agcy_name_x"], x["agcy_name_y"]), axis=1
    )
    ...
    enhanced_work_history = cleaned_df.copy()
    enhanced_work_history.to_csv(
        "../data/processed/ga-2024-enhanced-work-history.csv", index=False
    )

    index = enhanced_work_history.copy()
    index = index.drop(columns=["type"])
    index.to_csv("../data/processed/ga-2024-index.csv", index=False)
