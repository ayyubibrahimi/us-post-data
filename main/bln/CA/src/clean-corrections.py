...
def getargs():
    ...
    parser.add_argument('--input', default='output/ca-2023-original-corrections.csv')
...
if __name__ == '__main__':
    ...
    out = flat \
        .drop(['record_nbr', 'new_group', 'new_stint',
               'event_date', 'separation_date', 'stat05_ind',
               'event', 'matchable', 'stint_nbr'], axis=1, inplace=False) \
        .drop_duplicates()

    origlen = len(clean[['person_nbr', 'agcy_name', 'rank']].drop_duplicates())
    outlen = len(out[['person_nbr', 'agcy_name', 'rank']].drop_duplicates())
